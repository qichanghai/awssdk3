Description
---------------------------------------------
This module is copied from https://www.drupal.org/project/awssdk2, but use AWS SDK for PHP v3.

Installing the AWS SDK for PHP v3
---------------------------------------------

1. Read the "Installation and Usage For Site Builders" for https://drupal.org/project/composer_manager

2. Enable AWS SDK for PHP v3
   $ drush en awssdk3
...
3. Loading own configuration
  $data = awssdk3_service_load('s3');

  debug($data->config['my_value']);

4. Using AWS SDK for PHP v3 API

$client = awssdk3_get_client('s3');
if (!empty($client)) {
  debug($client);
}
